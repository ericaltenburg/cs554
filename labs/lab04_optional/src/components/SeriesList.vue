    ...
    <div>
        <div v-if="!this.badLoad">
            <div v-if="!this.isLastPage">
                <router-link :to="{path:'/series/page/' + (this.pageNum+1)}">Next Page</router-link>
            </div>
            <br />
            <div v-if="this.pageNum !== 0">
                <router-link :to="{path:'/series/page/' + (this.pageNum-1)}">Previous Page</router-link>
            </div>
            <ul>
                <li v-for="(series_indiv,index) in this.series" :key="index">
                    <router-link :to="{path:'/series/'+series_indiv.id}">{{series_indiv.title}}</router-link>
                </li>
            </ul>
        </div>
        <div v-else>
