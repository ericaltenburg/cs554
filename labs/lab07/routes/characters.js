...
const characterListSet = 'characterListSet';
...
router.get('/page/:page', async (req, res) => {
    ...
    try {
        ...
        const isInCache = await client.hexistsAsync(characterListSet, pageNum);
        if (isInCache) {
            let response = await client.hgetAsync(characterListSet, pageNum);
            ...
            await client.hsetAsync(characterListSet, pageNum, JSON.stringify(data.data.results));
...
router.get('/:id', async(req, res) => {
    ...
    try {
        ...
        const stringToHash = ts + privatekey + publickey;
        const hash = md5(stringToHash);
        const baseURL = `https://gateway.marvel.com:443/v1/public/characters/${charId}`;
        const URL = baseURL + '?ts=' + ts + '&apikey=' + publickey + '&hash=' + hash;

        const isInCache = await client.hexistsAsync(characterSet, charId);
        if (isInCache) {
            let response = await client.hgetAsync(characterSet, charId);
            response = JSON.parse(response);
            ...
            await client.hsetAsync(characterSet, charId, JSON.stringify(data.data.results[0]));
